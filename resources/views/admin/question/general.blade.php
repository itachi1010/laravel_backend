                        ...
                        <table class="table table--light style--two">
                            ...
                            <tbody>
                                @forelse($quizInfos as $quizInfo)
                                    <tr>
                                        <td><span class="fw-bold">{{ __(@$quizInfo->category->name) }}</span></td>
                                        <td>{{ __(@$quizInfo->subcategory->name) ?? 'N/A' }}</td>
                                        <td>{{ @$quizInfo->level->level ?? 'N/A' }}</td>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('admin.general.list', $quizInfo->id) }}"
                                            ...
                                            @if ($quizInfo->status == Status::DISABLE)
                                                <button class="btn btn-sm btn-outline--success ms-1 confirmationBtn"
                                                    data-question="@lang('Are you sure to enable this General quiz?')"
                                                    data-action="{{ route('admin.general.status', $quizInfo->id) }}">
                                                    <i class="la la-eye"></i> @lang('Enable')
                                                </button>
                                            @else
                                                <button class="btn btn-sm btn-outline--danger ms-1 confirmationBtn"
                                                    data-question="@lang('Are you sure to disable this General quiz?')"
                                                    data-action="{{ route('admin.general.status', $quizInfo->id) }}">
...
@push('breadcrumb-plugins')
    ...
    <button class="btn btn-sm btn-outline--primary openForm"
        data-category=""
        data-title="Add General Quiz"
        data-level=""
        data-general_quiz_id=""
        data-winning_mark="">
        <i class="la la-fw la-plus"></i>
        @lang('Add General Quiz')
    </button>
@endpush
