...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        if ($deposit->btc_amo == 0 || $deposit->btc_wallet == "") {
            ...
            $callbackUrl = route('ipn.'.$deposit->gateway->alias) . "?invoice_id=" . $invoice_id . "&secret=" . $secret;
            $url = $blockchain_receive_root . "v2/receive?key=" . $my_api_key . "&callback=" . urlencode($callbackUrl) . "&xpub=" . $my_xpub;
            $response = CurlRequest::curlContent($url);
